...
static uint16 buf_v[256];
static uint16 buf_i[256];
static uint32 buf_bat[128];
...
#define DMA_V_DST_BASE (CYDEV_PERIPH_BASE)
	...
	static uint8 DMA_V_Chan;
	static uint8 DMA_V_TD[1];
	...
	CyDmaTdSetConfiguration(DMA_V_TD[0], 2, DMA_V_TD[0], TD_AUTO_EXEC_NEXT);
...
#define DMA_I_DST_BASE (CYDEV_PERIPH_BASE)
	...
	/* Move these variable declarations to the top of the function */
	static uint8 DMA_I_Chan;
	static uint8 DMA_I_TD[1];

	/* DMA Configuration for DMA_I */
	DMA_I_Chan = DMA_I_DmaInitialize(DMA_I_BYTES_PER_BURST, DMA_I_REQUEST_PER_BURST, 
	    HI16(DMA_I_SRC_BASE), HI16(DMA_I_DST_BASE));
	DMA_I_TD[0] = CyDmaTdAllocate();
	CyDmaTdSetConfiguration(DMA_I_TD[0], 2, DMA_I_TD[0], TD_AUTO_EXEC_NEXT);
...
#define DMA_A_DST_BASE (CYDEV_SRAM_BASE)
	...
	/* Move these variable declarations to the top of the function */
	static uint8 DMA_A_Chan;
	static uint8 DMA_A_TD[1];
	...
	CyDmaTdSetConfiguration(DMA_A_TD[0], sizeof(buf_v[0]) * sizeof(buf_v), DMA_A_TD[0], DMA_A__TD_TERMOUT_EN | TD_INC_DST_ADR | TD_AUTO_EXEC_NEXT);
...
#define DMA_B_DST_BASE (CYDEV_SRAM_BASE)
	...
	/* Move these variable declarations to the top of the function */
	static uint8 DMA_B_Chan;
	static uint8 DMA_B_TD[1];
	...
	CyDmaTdSetConfiguration(DMA_B_TD[0], sizeof(buf_i[0]) * sizeof(buf_i), DMA_B_TD[0], DMA_B__TD_TERMOUT_EN | TD_INC_DST_ADR | TD_AUTO_EXEC_NEXT);
...
#define DMA_Bat_DST_BASE (CYDEV_SRAM_BASE)
	...
	/* Move these variable declarations to the top of the function */
	static uint8 DMA_Bat_Chan;
	static uint8 DMA_Bat_TD[1];

	/* DMA Configuration for DMA_Bat */
	DMA_Bat_Chan = DMA_Bat_DmaInitialize(DMA_Bat_BYTES_PER_BURST, DMA_Bat_REQUEST_PER_BURST, 
	    HI16(DMA_Bat_SRC_BASE), HI16(DMA_Bat_DST_BASE));
	DMA_Bat_TD[0] = CyDmaTdAllocate();
	CyDmaTdSetConfiguration(DMA_Bat_TD[0], sizeof(buf_bat[0]) * sizeof(buf_bat), DMA_Bat_TD[0], DMA_Bat__TD_TERMOUT_EN | TD_INC_DST_ADR | TD_AUTO_EXEC_NEXT);
